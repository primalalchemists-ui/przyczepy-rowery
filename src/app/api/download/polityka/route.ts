...
type BookingSettings = {
  politykaPrywatnosciPdf?: { url?: string | null } | number | string | null
}
...
export async function GET() {
  ...
  const fileUrl = resolveMediaUrl(booking?.politykaPrywatnosciPdf ?? null)

  if (!fileUrl) {
    return NextResponse.json({ error: 'Brak pliku polityki prywatności w ustawieniach.' }, { status: 404 })
  }
